...
func (r *Running) Calories() float64 {
	return ((float64(CaloriesMeanSpeedMultiplier) * r.meanSpeed() + CaloriesMeanSpeedShift) * r.Weight / float64(MInKm) * float64(r.Duration.Hours()) * float64(MinInHours)) 
}
...
func (w *Walking) Calories() float64 {
	result := math.Pow(w.meanSpeed(), 2)
	return ((CaloriesWeightMultiplier * w.Weight + result / w.Height)* CaloriesSpeedHeightMultiplier * w.Weight) * float64(w.Duration.Hours()) * float64(MinInHours)
}
